 ...
 * 
 * This parse extract only the node about the LOST call
 * 
 */
public class LostCallParser extends DefaultHandler{
	 ...
	 *  */
	public LostCallParser(DefaultMutableTreeNode b, JTree tree) {
	...
	public void startElement(String uri, String localName, String tagName, Attributes attr) throws SAXException {
		...
		for (int i = 0; i < attr.getLength(); i++) {
			...
			{												
				//type=2 --> made (sent); type=1 --> received ; type = 3 --> lost
				if(attr.getValue(i).equalsIgnoreCase("3")){
					appendNodeFlag = true;						   
				}
			}
		}
