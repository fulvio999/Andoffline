 ...
 * 
 * It read all the xml node and append it in the JTree without any filter
 * 
 */
public class SmsAllParser extends DefaultHandler{

	//The current node which appends the others ones
	private DefaultMutableTreeNode base;
	
	//The JTree to fill with the xml parsed
	...
	//The counter for the received sms
	private int smsReceivedCounter;
	 ...
	 * @param tree
	 */
	public SmsAllParser(DefaultMutableTreeNode b, JTree tree) {
				...
				{								
				 ...
				 //Convert the "type" attributes ie: type=2 --> sent; type=1 --> received	
						...
						if(attr.getValue(i).equalsIgnoreCase("1")){
							...
							currentAtt = new DefaultMutableTreeNode(labelToDisplay +"= Sent");
							base.add(currentAtt);
							 smsSentCounter ++;
	...
	public void endDocument() throws SAXException {
		...
		//Update the sent or receive amount in case of the user has filtered the sms
		//if(nodeToShow.equalsIgnoreCase("Received"))
			smsSentCounter = smsNodeCounter - smsReceivedCounter -1; //-1 because the smsNodeCounter start from 1
		
		//if (nodeToShow.equalsIgnoreCase("Sent"))
			smsReceivedCounter = smsNodeCounter - smsSentCounter -1;
		...
		tn.insert(smsReceivedCounterNode, 0);
	...
	public int getSmsReceivedCounter() {
		return smsReceivedCounter;
	}


	public void setSmsReceivedCounter(int smsReceivedCounter) {
		this.smsReceivedCounter = smsReceivedCounter;
	}
